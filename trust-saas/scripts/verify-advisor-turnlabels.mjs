/* ============================================================
   ...
   해결: 화면에 보이지 않는 `.sr-only` 라벨("내 질문."/"상담 답변.")을 각 턴
   첫머리에 넣어 발화자를 SR 에 고지한다(시각 UI 무변경).
...
const USER_LABEL = "내 질문.";
const ASSISTANT_LABEL = "상담 답변.";
...
console.log("\n[B] 사용자 턴 발화자 라벨");
{
  ...
  ok(/<span className="sr-only">내 질문\. <\/span>/.test(userSeg), "user 턴이 .sr-only \"내 질문.\" 라벨을 가짐");
...
console.log("\n[C] 정상 답변 턴 발화자 라벨 (.md 렌더 앞)");
{
  ...
  ok(/<span className="sr-only">상담 답변\. <\/span>/.test(normSeg), "정상 답변 턴이 .sr-only \"상담 답변.\" 라벨을 가짐");
...
console.log("\n[D] 오류 답변 턴도 발화자 라벨(턴 식별 일관)");
{
  ...
  ok(/<span className="sr-only">상담 답변\. <\/span>/.test(errSeg), "오류 턴이 .sr-only \"상담 답변.\" 라벨을 가짐");
  ok(/role="alert"/.test(errSeg), "오류 턴 role=\"alert\" 보존(실제 오류 즉시 낭독)");
}

console.log("\n[E] 라벨은 .sr-only(시각 숨김) — 화면 표시 무변경");
{
  // 발화자 문구가 .sr-only 스팬 안에서만 등장(가시 텍스트로 새지 않음).
  const userLabelSpans = (chat.match(/<span className="sr-only">내 질문\. <\/span>/g) || []).length;
  ok(userLabelSpans === 1, "\"내 질문.\" 라벨은 .sr-only 스팬 1회만(가시 노출 0)");
  const asstLabelSpans = (chat.match(/<span className="sr-only">상담 답변\. <\/span>/g) || []).length;
  ok(asstLabelSpans === 2, "\"상담 답변.\" 라벨은 .sr-only 스팬 2회(정상·오류 턴)만");
  // 발화자 문구가 .sr-only 밖 가시 텍스트로 등장하지 않는지(span 밖 누설 0)
  ok(!/[^>]내 질문\. /.test(chat.replace(/<span className="sr-only">내 질문\. <\/span>/g, "")),
    "\"내 질문.\" 이 .sr-only 밖 가시 텍스트로 새지 않음");
}
