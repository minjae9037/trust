...
console.log("\n[J] 부분집합 고지 — 입력 미완 제외 서류가 있으면 검수자에게 명시(표시 전용·스크립트 0)");
{
  const docs2 = [
    { name: "담보신탁계약서", html: "<body>가</body>" },
    { name: "위임장", html: "<body>나</body>" },
  ];
  // 제외 서류 있음 → notice 표출 + 개수 + 서류명 verbatim + 일괄 생성 제외 고지.
  const withEx = buildMultiDocShell(docs2, { excluded: ["고객거래확인서", '실제소유자확인서 "갑"'] });
  ok(/<div class="notice" role="note">/.test(withEx), "제외 있음: notice(role=note) 표출");
  ok(/입력 미완 2종은 이 검수에 포함되지 않았습니다/.test(withEx), "notice: 제외 개수(2종) 고지");
  ok(/일괄 생성\(\.docx\)·다운로드에서도 동일하게 제외/.test(withEx), "notice: 일괄 생성·다운로드도 제외 고지(부분 출하 방지)");
  ok(withEx.includes("〈고객거래확인서〉"), "notice: 제외 서류명 1 verbatim(〈〉)");
  // 큰따옴표는 텍스트 노드라 escText(&·<·>) 후에도 그대로 — verbatim.
  ok(withEx.includes('〈실제소유자확인서 "갑"〉'), "notice: 제외 서류명 2 verbatim(큰따옴표 보존)");
  ok(!/<script\b/i.test(withEx), "notice 포함 셸에도 <script> 0(순수 HTML·읽기 전용 유지)");
  // 제외 없음 → notice 미표출(전부 준비 완료 시 군더더기 없음·후방호환).
  const noEx0 = buildMultiDocShell(docs2);
  ok(!/class="notice"/.test(noEx0), "opts 미전달: notice 미표출(후방호환)");
  const noEx1 = buildMultiDocShell(docs2, { excluded: [] });
  ok(!/class="notice"/.test(noEx1), "excluded 빈 배열: notice 미표출");
  const noEx2 = buildMultiDocShell(docs2, { excluded: ["", "  ", "\t"] });
  ok(!/class="notice"/.test(noEx2), "excluded 공백뿐: notice 미표출(빈 이름 걸러짐)");
  // openMultiDocPreviewWindow 가 opts 를 셸로 전달.
  const { sink, win } = makeSink();
  openMultiDocPreviewWindow(docs2, () => win, { excluded: ["위임장"] });
  ok(/class="notice"/.test(sink.html) && sink.html.includes("〈위임장〉"),
    "openMultiDocPreviewWindow: opts.excluded 를 셸로 전달");
}
...
console.log("\n[H] 배선 — Wizard previewAllReady + 일괄 생성 보존 + 새 CSS 0");
{
  ...
  ok(/openMultiDocPreviewWindow\(\s*docs,/.test(body), "previewAllReady: 통합 창에 docs 전달");
  ok(/window\.open\(""\s*,\s*"_blank"/.test(body), "previewAllReady: window.open(_blank) 주입");
  // 부분집합 고지 — 입력 미완(미-ready) 서류 이름을 excluded 로 모아 opts 로 전달.
  ok(/docSteps\.filter\(\(s\) => !docReady\[s\.idx\]\)\.map\(\(s\) => s\.title\)/.test(body),
    "previewAllReady: 입력 미완 서류(미-ready) 이름을 excluded 로 수집");
  ok(/\{\s*excluded\s*\}/.test(body), "previewAllReady: opts {excluded} 를 통합 창에 전달(부분집합 고지)");
