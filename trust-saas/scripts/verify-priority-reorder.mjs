...
console.log("\n[E] StepPriority 배선(정적) — import·props 전달·최선순위·안내문");
{
  ...
  // 순서가 법적 의미를 갖는 역할에만 적용 — 우선수익자(선·후순위)·위탁자(대표위탁자=trustors[0]).
  // 채무자/수익자는 순서가 무의미하므로 orderable 미전달. 파일 전체에서 orderable 은
  // 정확히 2회(우선수익자 + 위탁자 PartyCard)만 등장해야 한다(verify-trustor-reorder 가 위탁자측 상세 검증).
  ok((sp.match(/orderable/g) || []).length === 2, "orderable 은 우선수익자·위탁자에만(파일 내 2회) — 채무자/수익자 미적용");
}
