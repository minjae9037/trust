...
console.log("\n[G] 배선 — JointForm 우측 실시간 2분할 미리보기(담보신탁 DocStep 동형)");
{
  ...
  // 외곽 클래스는 미리보기 접기 토글 도입(verify-jointform-preview-collapse) 후
  // `previewOpen ? "doc-split" : "doc-split doc-split--preview-collapsed"` 조건부가 됐다.
  // 의도(=2분할 doc-split 레이아웃 도입)는 그대로 — 펼침 분기의 "doc-split" 리터럴로 앵커.
  ok(/className=\{previewOpen \? "doc-split"/.test(joint),
    "JointForm: 2분할 레이아웃(.doc-split) 도입(미리보기 접기 토글 조건부 클래스의 펼침 분기)");
