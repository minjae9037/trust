...
console.log("\n[D] 무접촉/무회귀 — showLoanFields 내부·onChange 보존·새 CSS 0·조문 import 0");
{
  ...
  // ★대출금액 readback 특정 매칭(loan-hangul 은 생년월일 readback 등 다른 입력도 재사용하므로
  //   amountToHangul(party.loanAmount) 로 한정) — 이 loan readback 이 showLoanFields 블록 이후여야 한다.
  const rb = partyCard.indexOf('className="loan-hangul" role="status" aria-live="polite">{amountToHangul(party.loanAmount)}');
  ok(rb > sl, "대출금액 readback 이 showLoanFields 블록 이후(우선수익자 전용 — 위탁자/채무자 카드 무영향)");
