...
console.log("\n[B] ★영속 라이브 영역 — role=status·aria-live=polite·aria-atomic, ocrMsg 게이트 밖");
{
  ...
  // OCR 영속 라이브 영역은 1곳만(과다 영역 회귀 감지). ★면적 확인용 readback 도 role="status"
  // 를 쓰므로(loan-hangul·금액 readback 패턴, aria-atomic 없음) role 개수가 아니라 OCR 영역의
  // 구별 속성인 aria-atomic="true" 로 카운트한다 — OCR 영속 영역의 중복만 정확히 감지.
  ok((src.match(/aria-atomic="true"/g) || []).length === 1, "OCR 영속 라이브 영역(aria-atomic) 정확히 1곳");
