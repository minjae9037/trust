...
console.log("\n[H] ★복사 결과 스크린리더 고지(WCAG 4.1.3) — 폴라이트 라이브 영역 재사용");
{
  ...
  // announce 헬퍼 = setLiveMsg("") 로 비운 뒤 setTimeout 으로 세팅(동일 문구 연속 고지 재낭독 보장).
  // ★컴포넌트 스코프 공용 헬퍼로 승격(복사·피드백 공용) — copyAnswer 안이 아니라 파일(컴포넌트)에 1개.
  ok(/const\s+announce\s*=\s*\(text[^)]*\)\s*=>\s*\{/.test(chat), "announce(text) 공용 헬퍼 존재(컴포넌트 스코프)");
  ...
  ok((chat.match(/const\s+announce\s*=/g) || []).length === 1, "announce 헬퍼 정의 정확히 1곳(중복 정의 없음)");
