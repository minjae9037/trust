/* ============================================================
   ...
   핵심 불변식:
     - 복사 대상 = parseAction 으로 내부 액션 마커(<<doc:…>>)가 제거된 `body`.
       원시 `m.content`(마커 포함 가능)를 복사하지 않는다(마커 비노출 계약 유지).
     - 복사 버튼은 답변이 완성됐을 때만 노출(body && !busy) — 스트리밍 중 미노출.
     - navigator.clipboard 미지원/거부는 조용히 무시(전송·답변 경로 무영향).
     - 표시 전용 — 조문·엔진·산출물·검색·로깅·페르소나 무접촉.

   단언:
     (A) copyAnswer 존재 + navigator.clipboard.writeText 사용 + 실패 try/catch
...
console.log("\n[A] copyAnswer — 클립보드 복사 + 실패 무시");
{
  ...
  ok(/navigator\.clipboard\.writeText\(/.test(chat), "navigator.clipboard.writeText 사용");
