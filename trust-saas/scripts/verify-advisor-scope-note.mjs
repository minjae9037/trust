...
console.log("\n[F] route.ts 가 순수 buildAdvisorSystem 사용·인라인 조립 잔존 0");
{
  ...
  ok(/import\s*\{\s*buildAdvisorSystem\s*\}\s*from\s*["']@\/lib\/advisor\/system["']/.test(route), "buildAdvisorSystem import");
  ok(route.includes("buildAdvisorSystem(ADVISOR_PERSONA, contextText)"), "조립을 순수 함수에 위임");
