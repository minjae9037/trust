...
{
  // react named import 에 useRef 가 포함되는지만 단언(목록 순서·추가 import 에 내성).
  // 종전엔 정확한 목록 `{ useEffect, useRef, useState }` 를 박아, 무관한 import 추가
  // (예: useSyncExternalStore)에도 깨졌다 — 이 가드의 의도는 "useRef 가 import 됨"이다.
  ok(/import \{[^}]*\buseRef\b[^}]*\} from "react"/.test(app), "useRef import 추가(react named import 포함)");
