...
console.log("\n[E] ★정확성 — 안내가 실제 파일명 식별부(위탁자·체결일·소재지)와 일치(제목 오안내 금지)");
{
  // 파일명 = `{서류종류명}_{위탁자}_{체결일}_{소재지}` = docFileBase/contractFileKey.
  // 계약 "제목"은 파일명에 들어가지 않으므로(카드의 이름변경=제목 변경), 충돌 안내가
  // 제목을 식별 항목으로 권하면 사용자가 이름변경으로 헛되이 해소를 시도하게 된다(오안내).
  // 시각 경고 줄·SR openLabel 두 출처 모두 실제 식별부(위탁자·체결일·소재지)를 안내하고
  // "제목과 무관"을 명시하며, 제목을 구분 항목으로 권하지 않아야 한다(정확성 불변식).
  const vAt = view.indexOf("{collision && (");
  const vBlock = view.slice(vAt, vAt + 460);
  const oAt = view.indexOf("const collisionLabel = collision");
  const oBlock = view.slice(oAt, oAt + 420);
  // 긍정: 두 출처 모두 실제 파일명 식별부를 안내
  ok(/위탁자·체결일·담보물건 소재지로 정해집니다/.test(vBlock),
     "시각 경고: 파일명은 위탁자·체결일·담보물건 소재지로 정해진다고 안내");
  ok(/위탁자·체결일·담보물건 소재지로 정해집니다/.test(oBlock),
     "SR openLabel: 동일하게 위탁자·체결일·담보물건 소재지 안내(시각과 동일 출처)");
  // 긍정: "계약 제목과 무관" 명시(제목 변경으로 안 풀림을 알림)
  ok(/계약 제목과 무관/.test(vBlock) && /계약 제목과 무관/.test(oBlock),
     "두 출처 모두 '계약 제목과 무관' 명시(이름변경 오해 차단)");
  // 긍정: 같은 계약(값 동일)이면 받은 파일명 직접 변경이라는 실제 해소책 안내
  ok(/받은 .*파일 이름을 직접 바꿔/.test(vBlock) && /받은 파일 이름을 직접 바꿔/.test(oBlock),
     "두 출처 모두 같은 계약이면 받은 파일명 직접 변경 안내(actionable)");
  // ★음성(오안내 회귀 잠금): 제목을 파일명 식별 항목으로 권하지 않음
  //   옛 문구 "제목·체결일·담보물건 소재지를/로 구분" 형태가 되살아나지 않게 한다.
  ok(!/제목·체결일/.test(vBlock) && !/제목·체결일/.test(oBlock),
     "음성: 옛 '제목·체결일…' 열거(제목을 식별 항목으로 권유) 부재");
  ok(!/제목[^과]{0,20}구분/.test(vBlock) && !/제목[^과]{0,20}구분/.test(oBlock),
     "음성: '제목…구분' 권유 부재(제목 변경 오안내 금지)");
}
