...
console.log("\n[G] joint 배선 — JointForm.onPdf 가 generateJointPDFDoc 반환값으로 분기(거짓 성공 차단)");
{
  ...
  // 성공 메시지가 opened 분기(삼항 또는 if(opened) 블록) 안에만 있다 — 무조건 성공표시가 아니다.
  // (생성 신선도 추가로 ternary→if/else 로 구조 변경됐으나 불변식 동일: opened===true 일 때만 성공.)
  ok(/(opened\s*\?[\s\S]*?인쇄창을 열었습니다|if \(opened\) \{[\s\S]*?인쇄창을 열었습니다)/.test(jointForm),
