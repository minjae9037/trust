...
function commonMissing(form: ContractForm): Missing[] {
  ...
  form.properties.forEach((p, i) => {
    ...
    // 정량 산출 필드이나 게이트가 유일하게 검사하지 않아, "0·음수·비숫자"인 면적(예: -50·abc)이
    // 법적 표에 "-50㎡"·"NaN㎡"로 들어갈 수 있던 마지막 정량 입력 갭(가격·원본가액 패턴과 동일).
    // ⚠️ 빈 값은 차단하지 않는다(면적은 선택 입력 — 빌더가 빈칸으로 렌더, 미입력 계약 무회귀).
